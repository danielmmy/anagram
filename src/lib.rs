...
use std::{char, collections::HashMap, hash::BuildHasherDefault};
...
pub fn is_anagram_nohash_hashmap(word1: impl Into<String>, word2: impl Into<String>) -> bool {
    let word1: Vec<char> = Into::<String>::into(word1).chars().collect();
    let word2: Vec<char> = Into::<String>::into(word2).chars().collect();
    if word1.len() != word2.len() {
        return false;
    ...
    let mut map = HashMap::<char, i32, BuildHasherDefault<NoHash<char>>>::default();
...
mod tests {
    ...
    #[test]
    fn is_anagram_nohash_hashmap_tests() {
        // arrange
        let test_table: Vec<(&'static str, &'static str, bool)> = vec![
            ("abc", "cba", true),
            ("abc", "caa", false),
        ...
        for (word1, word2, expected) in test_table {
            // act
            let res = is_anagram_nohash_hashmap(word1, word2);
